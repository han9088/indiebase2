...
- Dashboard 用户访问 Data API：`/api/data/*` + **Dashboard Session** + 请求头 **`X-Indiebase-Project-Id`**（每请求查 `project_members`；**无**第二套 Project Session）
...
- **用户登录统一 Opaque Token + Redis**（Dashboard Session、App User Session）；**不使用 JWT**；**不使用**独立的 Project Session
...
| 数据 | 访问方式 | 说明 |
|------|----------|------|
| Platform 表（`public.*`） | **sqlx + SeaQuery** | 开发：`db/schema.rs` synchronize；生产：sqlx migrations；users、projects、api_keys、metadata |
...
/api/auth/login
/api/auth/logout
/api/auth/project-context          ← Dashboard Session + X-Indiebase-Project-Id → role
/api/auth/app/login
/api/auth/app/logout
/api/projects
...
- 解析 project 上下文（`X-Indiebase-Project-Id` + `project_members`，见 §11.8）
...
Axum 透明代理至内网 PostgREST，**不重复实现** CRUD 逻辑。Dashboard 与 SDK **路径不同**：Dashboard URL **不含** `project_id`（project 来自请求头 `X-Indiebase-Project-Id`）；SDK 侧 URL **含** `{project_id}`（project 由路径指定，Key 只做鉴权）。

#### 6.2.1 Dashboard 用户（Row Viewer）

Dashboard 用户访问 Data API 使用 **同一 Dashboard Session**（§11.7），并在每请求携带 **`X-Indiebase-Project-Id`**。网关查 `project_members` 解析 `project_role`。**不是 API Key，也不是第二套 Project Session。**
...
Authorization: Bearer <dashboard_session_token>
X-Indiebase-Project-Id: <project_ulid>
```
...
GET /api/data/users?select=*
    ...
    ▼
Data API 网关 (Axum)
    │  1. Redis lookup  dashboard_session:{token} → { user_id, … }
    │  2. 读取 X-Indiebase-Project-Id；查 project_members → project_role
...
前置：仅需 Dashboard 登录（§11.7）。切换项目 = 换请求头，**不换 token**。

#### 6.2.2 SDK / 程序化调用

SDK **不走 Dashboard Session**。**Project 由 URL 中的 `{project_id}` 指定**；凭证与权限分层见 §11.2、§11.3。
...
| 分支 | project 来源 | 忽略 |
|------|--------------|------|
| Dashboard | 请求头 `X-Indiebase-Project-Id` + `project_members` | URL 中即使出现 ULID 形 segment 也不作 project 解析（SDK 分支已拦截） |
| SDK | URL `{project_id}` | **不**读取 Dashboard Session |

**凭证互斥（非法组合 → `403` + 统一错误体）：**

| 分支 | 必须 | 禁止 |
|------|------|------|
| Dashboard `/api/data/*` | `Authorization: Bearer <dashboard_session_token>` + `X-Indiebase-Project-Id` | `X-Indiebase-Api-Key`（任意 Key）；Bearer 为 App User Session |
| SDK `/api/data/{project_id}/*` | `X-Indiebase-Api-Key`（Publishable 或 Secret） | Dashboard Session（Dashboard 凭证不得用于 SDK 路径） |
...
网关解析 `Authorization: Bearer` 时：**先按 URL 分支决定**允许哪种 Session；在 SDK 分支 **仅** lookup `app_user_session:`，**不得**把 Dashboard Session 当作用户身份。

**为何不会常态冲突：** Dashboard 客户端 **不**调用含 `{project_id}` 的 Data API URL；SDK 客户端 **不**持有 Dashboard Session。混用仅可能来自错误集成或恶意请求，由上述规则 **拒绝**。
...
| Row Viewer | **Data API** `/api/data/*` | **Dashboard Session** + `X-Indiebase-Project-Id`；见 §6.2.1、§11.8 |
...
**Row Viewer** — 用户持 **Dashboard Session**，每请求带 `X-Indiebase-Project-Id` 访问 Data API：

```
Dashboard → GET /api/data/users?…
            Authorization: Bearer <dashboard_session_token>
            X-Indiebase-Project-Id: <project_ulid>
            → project_members → Data API 网关 → PostgREST → PostgreSQL
```
...
- Project 上下文（Dashboard）：请求头 **`X-Indiebase-Project-Id`** + `project_members`；Data API URL 为 `/api/data/*`
...
│
├── public                    ← 平台层（Axum 直连）
│   ├── users                 ← soft delete: deleted_at；活跃邮箱部分唯一
│   ├── projects              ← soft delete: deleted_at
│   ├── project_members       ← soft delete: deleted_at
│   ├── api_keys              ← project_id, key_type (publishable|secret), key hash, status; soft delete: deleted_at
...
| 与用户凭证关系 | Key **不是**终端用户登录凭证；用户身份由 **App User Session**（SDK）或 **Dashboard Session**（Dashboard，另加 `X-Indiebase-Project-Id`）单独承载，走 `Authorization: Bearer`（RFC 6750） |
...
| `X-Indiebase-Project-Id` | **Dashboard project 上下文**（非 Key） | 26 字符 project ULID；仅 Dashboard 路径 |
...
Dashboard 用户访问 Data API 用 **Dashboard Session + `X-Indiebase-Project-Id`**（§11.8），**不用 API Key**。
...
| **用户身份** | App User Session（SDK）；Dashboard Session + project 头（Row Viewer） | SDK：租户 **App 用户**；Row Viewer：**平台成员** + `project_role` |
...
**Dashboard project 上下文（概要）：**

- 平台成员身份 + `project_role`（owner / admin / member），来自 `public.project_members`（每请求解析）
...
- Dashboard Data API 路径 **永不**读取租户 `users` 表来识别操作者；操作者始终是 `public.users` + `project_members.project_role`
- owner / admin 的「全表 CRUD」是 **Dashboard 运维能力**，与 Secret Key 数据面能力等价但凭证为 Dashboard Session（须审计）
...
- Dashboard Data API owner/admin：全表 CRUD；member：只读
...
| Session | Dashboard / App User Session 为 Opaque Token + Redis；TTL / 滑动续期 / 登出即删 Redis；**分支互斥**见 §6.2.3（SDK：App User Session `project_id` 须与 URL 一致；Dashboard：Bearer + `X-Indiebase-Project-Id`） |
...
### 11.8 Project 上下文（请求头 — 非独立 Session）

Dashboard **不**再签发第二套 Project Session。Project 上下文由请求头携带，**每请求**用 Dashboard Session 的 `user_id` 查 `project_members`。

| 属性 | 说明 |
|------|------|
| 用途 | Dashboard 访问 **Data API** / 其他 project-scoped 路由 |
| 前置 | 有效 **Dashboard Session**（§11.7） |
| 传递 | `X-Indiebase-Project-Id: <project_ulid>`（与 `Authorization: Bearer` 同请求） |
| 解析 | `project_members` → `project_role`（`owner` \| `admin` \| `member`） |
| 调试 | `GET /api/auth/project-context` 返回 `{ project_id, role }` |
| 嵌套 Manager 路由 | `/api/projects/{project_id}/…` 可从 **URL** 取 project，仍用 Dashboard Session + 成员校验 |
...
X-Indiebase-Project-Id: <project_ulid>
        │
        ▼
   Redis  dashboard_session:{token} → { user_id, … }
        │
        ▼
...
**`X-Indiebase-Project-Id` 是 Dashboard 用户侧 project 上下文的来源**（非 Redis 第二 session）。切换项目只改 header，不重新登录。
...
App 终端用户（Project 租户 schema 内的 `users` 等）通过 **Project Auth** 登录，使用 **Opaque Token + Redis**（与 Dashboard Session **同一技术**，**作用域为单个 project 的终端用户**）。**这是 SDK 客户端访问 Data API 时携带的用户身份凭证。**
...
| Dashboard 数据 | `/api/data/*` | Dashboard Session | **`X-Indiebase-Project-Id`** + `project_members` | `project_operator` / `project_operator_readonly`（§11.3） |
...
| Dashboard + project 头，owner / admin | `project_operator` | 租户 `project_operator` | bypass App 用户 RLS |
| Dashboard + project 头，member | `project_operator_readonly` | 租户 `project_operator_readonly` | 仅 SELECT |
...
| Auth 模块 | 新增 **Project Auth**（App User Session 签发 / 吊销）；与 Dashboard Session **同一 Opaque Token 技术** |
...
请求进入 Data API 网关
    ...
    │
    ├─ Secret Key ──────────────► service
    ├─ Dashboard + X-Indiebase-Project-Id ► project_operator | project_operator_readonly
...
交付里程碑与验收见 **[mvp-phases.md](./mvp-phases.md)**。OpenSpec 按能力组织（见该文档「与 OpenSpec」与 `openspec/specs/`），不以 Phase 编号命名 change。
