...
| MVP scope | [mvp-sdk.md](./mvp-sdk.md) · 交付清单 [mvp-phases.md](./mvp-phases.md) |

[MVP 架构](./baas-platform-architecture.md) 交付 bootstrap RLS + `SET ROLE`（§11.3、§11.11）。**本文件**记录 MVP 之后按依赖顺序推进的能力；实现状态在此维护。

OpenSpec **按能力命名 change**（见 [mvp-phases.md「与 OpenSpec」](./mvp-phases.md) / `openspec/config.yaml`），**不**用 Phase 编号。本文件条目对应的建议 capability / change 名：

| # | 建议 change 名 |
|---|----------------|
| 1–3 | `abac-policies`（可再拆 `policy-compiler`、`authenticated-deny-default`） |
| 4 | `row-viewer-impersonation` |
| 5 | `multi-platform-sdk` |
| 6 | `client-storage` |
| 7 | `data-api-extensions` |
| 8 | `graphql-gateway` |
...
- Dashboard 客户端（Session + `X-Indiebase-Project-Id`）
...
- [MVP 交付清单 · OpenSpec 能力命名](./mvp-phases.md)
